...
internal class Basic_Item_Rando
{
    ...
    private static void OnSceneLoad(Scene scene, LoadSceneMode _)
    {
        ...
        foreach (SavedItem savedItem in Resources.FindObjectsOfTypeAll<SavedItem>())
        {
            ...
            {
                savedItemDictionary.Add(savedItem.GetType().ToString(), [savedItem]);
            }
        }
    ...
    private static void RecursiveObjectUpdater(GameObject[] objects, bool blackThread = false, bool normalWorld = false)
    {
        ...
        if (blackThread)
        {
            workingScene += "(BlackThread)";
        }

        if (normalWorld)
        {
            workingScene += "(NormalWorld)";
        }
        ...
        foreach (GameObject obj in objects)
        {
            ...
            for (int i = 0; i < obj.transform.childCount; i++)
            {
                ...
                {
                    if (normalWorld || obj.transform.GetChild(i).gameObject.name.Equals("Normal World"))
                    {
                        nwObjectChildren.Add(obj.transform.GetChild(i).gameObject);
                    }
                    else
                    {
                        objectChildren.Add(obj.transform.GetChild(i).gameObject);
                    }
                }
            }
            ...
            if (!added)
            {
                    ...
                    obj.layer.Equals(30)) // rosary string pieces?
                {
                    ...
                    {
                        ignoredObjects.Add(obj.layer, [obj]);
                    }
                }
                else
                {
                    ...
                    {
                        unprocessedObjects.Add(obj.layer, [obj]);
                    }
                }
            }
            else
            {
                ...
                {
                    lastObjectSet.Add(obj.layer, [obj]);
                }
            }
        }
        ...
        {
            RecursiveObjectUpdater([.. objectChildren]);
        }

        if (btObjectChildren.Count > 0)
        {
            RecursiveObjectUpdater([.. btObjectChildren], blackThread: true);
        }

        if (nwObjectChildren.Count > 0)
        {
            RecursiveObjectUpdater([.. nwObjectChildren], normalWorld: true);
        }
    }
    ...
    private static void LocalRandomize()
    {
        ...
        foreach (KeyValuePair<int, HashSet<GameObject>> gameObjectSet in lastObjectSet)
        {
            foreach (GameObject obj in gameObjectSet.Value)
            {
    ...
    private static void AddToObjectHolders(GameObject obj, Type componentType, string currentScene)
    {
        ...
        void Adder(SavedItem item)
        {
            ...
            {
                objectHolders.Add(new MultiObjectHolder(item, [new Location(currentScene, componentType.Name, obj.name)], 1));
            }
        }
    }
    ...
    private static void AddToList(string scene, GameObject obj, Dictionary<string, HashSet<string>> list, ref bool added)
    {
        ...
        {
            list.Add(scene, [obj.name]); // add this scene into the list
        }
